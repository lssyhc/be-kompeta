...
class UpdateProfileRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        if ($user->role === User::ROLE_SEKOLAH) {
            return array_merge([
                'user' => ['nullable', 'array:email'],
                'user.email' => ['sometimes', 'email', 'max:255', Rule::unique('users', 'email')->ignore($user->id)],
        ...
        if ($user->role === User::ROLE_MITRA && $user->mitra_type === User::MITRA_PERUSAHAAN) {
            return array_merge([
                'user' => ['nullable', 'array:name,email'],
                'user.name' => ['sometimes', 'string', 'max:255'],
                'user.email' => ['sometimes', 'email', 'max:255', Rule::unique('users', 'email')->ignore($user->id)],
                'profile' => ['sometimes', 'array:company_name,industry_sector,employee_total_range,office_address,socials,short_description'],
                'profile.company_name' => ['sometimes', 'string', 'max:255'],
        ...
        if ($user->role === User::ROLE_MITRA && $user->mitra_type === User::MITRA_UMKM) {
            return array_merge([
                ...
                'user.name' => ['sometimes', 'string', 'max:255'],
                'user.email' => ['sometimes', 'email', 'max:255', Rule::unique('users', 'email')->ignore($user->id)],
                'profile' => ['sometimes', 'array:business_name,owner_personal_nib,business_type,business_address,socials,short_description'],
                'profile.business_name' => ['sometimes', 'string', 'max:255'],
